...
class CraftMyPdfService {
  ...
  private static readonly API_KEY = '4784MTU4OTU6MTU5NzU6SGZHeWlnMmk3NU1ZWDZvNg=';
  private static readonly TEMPLATE_ID = '9df77b23bb4ca818';

  static async generateRncPdf(rnc: NonConformity): Promise<Blob> {
    try {
      // Calcular totais
      const totalMaoObra = rnc.workforce?.reduce((total, worker) => {
        const valor = parseFloat(worker.hours) * 20; // Valor exemplo por hora
        return total + valor;
      }, 0) || 0;

      const totalMaterial = rnc.materials?.reduce((total, material) => {
      ...
      const data: CraftMyPdfData = {
        ...
        data_rnc: format(new Date(rnc.dateOccurrence), 'dd/MM', { locale: ptBR }),
        ...
        prazo: rnc.dateConclusion ? format(new Date(rnc.dateConclusion), 'dd/MM', { locale: ptBR }) : '',
        ...
        imagens: rnc.images?.map((image, index) => ({
          key: index + 1,
          url: image.url.startsWith('http') ? image.url : `https://api.gmxindustrial.com.br${image.url}`
        })) || [],
        mao_obra: rnc.workforce?.map((worker, index) => {
        ...
        material: rnc.materials?.map((material, index) => {
