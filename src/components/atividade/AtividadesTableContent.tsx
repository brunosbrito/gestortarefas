...
interface AtividadesTableContentProps {
  atividades: AtividadeStatus[];
  filtros: AtividadeFiltros;
  isLoading?: boolean;
}

export const AtividadesTableContent = ({
  atividades,
  filtros,
  isLoading = false,
}: AtividadesTableContentProps) => {
  ...
  const handleExportPDFAdvanced = async (config: any) => {
    setIsExporting(true);
    try {
      await AtividadePdfAdvancedService.downloadPDF(atividades, filtros, config);
      toast({
        title: 'PDF Gerado',
  ...
  const handleExportExcel = async (config: ExcelConfig) => {
    setIsExporting(true);
    try {
      await AtividadeExcelService.downloadExcel(atividades, filtros, config);
      toast({
        title: 'Excel Gerado',
